...
class Layer(object):
    ...
    def add_output(self, shape, dim_names, out_name=None, var_name='layer{index}_out.data', type_name='layer{index}_t', precision=None, pragma='auto'):
        if out_name is None:
            out_name = self.outputs[0]
...
class Input(Layer):
    def initialize(self):
        ...
        self.add_output_variable(shape, dims, var_name=self.name+'.data', type_name='input_t')
