  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
...
#ifdef MNTR_CATAPULT_HLS
      CCS_DESIGN(econV1)(input_1,layer10_out,size_in1,size_out1);
#else
      ...
      if (e % CHECKPOINT == 0) {
        ...
        for(int i = 0; i < OUT_HEIGHT_10*OUT_WIDTH_10*N_FILT_10; i++) {
          std::cout << layer10_out[i].to_double() << " ";
        }
        std::cout << std::endl;
      }
    ...
    fout_ifbm << std::endl;
    for(int i = OUT_HEIGHT_10*OUT_WIDTH_10*N_FILT_10-1; i >= 0; i--) {
