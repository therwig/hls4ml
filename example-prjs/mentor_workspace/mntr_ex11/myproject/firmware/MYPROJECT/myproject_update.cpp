...
void myproject_2d(
    ...
    for(unsigned i0 = 0; i0 < N_INPUT_1_1; i0++) { 
      ...
      nnet::reset_down_2dXNew<layer2_t,layer2_t,config2>(i0,layer2_in_row,layer2_in);
      nnet::reset_down_2dXNew<layer4_t,layer4_t,config4>(i0,layer4_in_row,layer4_in);
      ...
      for(unsigned i1 = 0; i1 < N_INPUT_2_1; i1++) { 
       ...
       if(i0 % config6::stride_height == 0 && i1 % config6::stride_width == 0 && i0 < config6::out_width && i1 < config6::out_height) {
        nnet::fill_image_2dFull<layer6_t,result_t,config6>(pX,pY,layer6_out,layer7_in);
       }
       //Shift the whole row down by moving everything up and making it zeros
       nnet::shift_right_2dNew<layer2_t,layer2_t,config2>(i0,i1,layer2_in_row,layer2_in);//add padding
       nnet::shift_right_2dNew<layer4_t,layer4_t,config4>(i0,i1,layer4_in_row,layer4_in);//add padding
       nnet::shift_right_2dNew<layer6_t,layer6_t,config6>(i0,i1,layer6_in_row,layer6_in);//add padding
       if(i1 % config6::stride_height == 0) pX = pX+1;
     }
     if(i0 % config6::stride_height == 0) pY = pY+1;
   }
